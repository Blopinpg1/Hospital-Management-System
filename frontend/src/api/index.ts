...
import type {
  ...
  Patient, CreatePatientInput, PatientSummary,
  Doctor, Department,
  Appointment, TodaysAppointment, DoctorSlots, BookAppointmentInput,
  Admission, BedOccupancy, OccupancySummary, WaitingListEntry, AdmitPatientInput,
  MedicalRecord, Prescription, LabResult, LabTest,
  Bill, OutstandingBill, DoctorRevenue, MonthlyRevenue,
  Medicine, StockReport,
...
export const authApi = {
  ...
  changePassword: (current_password: string, new_password: string) =>
    api.patch<ApiResponse>('/auth/change-password', { current_password, new_password }),
};
...
export const doctorsApi = {
  ...
  schedule: (id: number) =>
    api.get<ApiResponse<{ doctor: Doctor; schedule: Appointment[] }>>(`/doctors/${id}/schedule`),
};
...
export const appointmentsApi = {
  today: () =>
    api.get<ApiResponse<TodaysAppointment[]>>('/appointments/today'),
  upcoming: (days = 7) =>
    api.get<ApiResponse<Appointment[]>>('/appointments/upcoming', { params: { days } }),
  byDate: (date: string) =>
    api.get<ApiResponse<Appointment[]>>('/appointments/by-date', { params: { date } }),
  slots: (doctor_id: number, date: string) =>
    api.get<ApiResponse<DoctorSlots>>('/appointments/slots', { params: { doctor_id, date } }),
  get: (id: number) =>
...
export const admissionsApi = {
  ...
  active: () =>
    api.get<ApiResponse<Admission[]>>('/admissions/active'),
  bedOccupancy: () =>
    api.get<ApiResponse<{ beds: BedOccupancy[]; summary: OccupancySummary[] }>>('/admissions/bed-occupancy'),
  availableBeds: () =>
    api.get<ApiResponse<BedOccupancy[]>>('/admissions/available-beds'),
  waitingList: () =>
    api.get<ApiResponse<WaitingListEntry[]>>('/admissions/waiting-list'),
  overdue: () =>
    api.get<ApiResponse<Admission[]>>('/admissions/overdue'),
  get: (id: number) =>
    api.get<ApiResponse<Admission>>(`/admissions/${id}`),
  admit: (data: AdmitPatientInput) =>
    api.post<ApiResponse<Admission>>('/admissions', data),
  addWaiting: (data: {
    patient_id: number; doctor_id: number; room_type_id: number;
    visit_type: string; reason?: string;
  ...
  discharge: (id: number, summary?: string, discount_pct?: number) =>
    api.post<ApiResponse>(`/admissions/${id}/discharge`, { summary, discount_pct: discount_pct ?? 0 }),
};

// ─── Clinical ─────────────────────────────────────────────────────────────────
export const clinicalApi = {
  records: (patient_id: number) =>
    api.get<ApiResponse<MedicalRecord[]>>('/clinical/records', { params: { patient_id } }),
  record: (id: number) =>
  ...
  prescriptions: (patient_id: number) =>
    api.get<ApiResponse<Prescription[]>>('/clinical/prescriptions', { params: { patient_id } }),
  ...
  labTests: () =>
    api.get<ApiResponse<LabTest[]>>('/clinical/lab-tests'),
  labResults: (patient_id: number) =>
    api.get<ApiResponse<LabResult[]>>('/clinical/lab-results', { params: { patient_id } }),
  pendingLab: () =>
    api.get<ApiResponse<LabResult[]>>('/clinical/lab-results/pending'),
  orderLab: (data: { patient_id: number; record_id?: number; test_id: number; ordered_date: string }) =>
    api.post<ApiResponse<LabResult>>('/clinical/lab-results', data),
  updateLab: (id: number, data: { result_value?: string; result_date?: string; status: string; remarks?: string }) =>
...
export const billingApi = {
  ...
  bill: (id: number) =>
    api.get<ApiResponse<Bill>>(`/billing/bills/${id}`),
  create: (data: { patient_id: number; appointment_id?: number; admission_id?: number; discount_pct?: number }) =>
    api.post<ApiResponse<Bill>>('/billing/bills', { discount_pct: 0, ...data }),
  payment: (id: number, data: { amount: number; method: string; reference_no?: string }) =>
    api.post<ApiResponse>(`/billing/bills/${id}/payment`, data),
  outstanding: () =>
    api.get<ApiResponse<OutstandingBill[]>>('/billing/outstanding'),
  doctorRevenue: (year?: number, month?: number) =>
    api.get<ApiResponse<DoctorRevenue[]>>('/billing/revenue/doctor', { params: { year, month } }),
  monthlyRevenue: (year?: number) =>
    api.get<ApiResponse<MonthlyRevenue[]>>('/billing/revenue/monthly', { params: { year } }),
};
...
export const inventoryApi = {
  ...
  lowStock: () =>
    api.get<ApiResponse<Medicine[]>>('/inventory/stock/low'),
  stockReport: () =>
    api.get<ApiResponse<StockReport[]>>('/inventory/stock'),
